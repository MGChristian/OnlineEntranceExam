                    ...
                    <table>
                        ...
                        <?php
                            if(isset($_POST['search'])){
                                $search = $_POST['search'];
                                $finder = "SELECT * FROM Student_Accounts INNER JOIN Courses ON Student_Accounts.Course_ID = Courses.Course_ID WHERE FirstName = '$search' OR LastName = '$search'";
                                $finderrun = mysqli_query($conn, $finder);
                                if(mysqli_num_rows($finderrun) > 0){
                                ...
                                if($search == ""){
                                    while($tablestudentsrow = mysqli_fetch_assoc($tablestudentsrun)){
                                        ...
                                        echo"
                                                ...
                                                <td>{$tablestudentsrow['Password']}</td>
                                                <td class='table-btn'><form method='POST' action='view.php'><input type='text' name='studdet' hidden value='$ref'><button type='submit' class='views'>VIEW</button></form></td>
                                                </tr>";
                                        }
                                }
